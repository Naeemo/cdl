...
1. **字段名不匹配** → 检查 Data 定义和 Chart 中的 x/y/group 是否一致
2. **缺少 use 语句** → Chart 必须包含 "use DataName"
3. **类型不支持** → 使用 16+ 种支持的图表类型
4. **数据格式错误** → Data 块必须是 CSV 格式（逗号分隔）
