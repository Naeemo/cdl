...
function activate(context) {
    ...
    const completionProvider = vscode.languages.registerCompletionItemProvider('cdl', {
    ...
    const validateDocument = (document) => {
        ...
        lines.forEach((line, i) => {
            ...
            if (line.match(/^Data\s*\{/)) {
                ...
                diagnostics.push(new vscode.Diagnostic(range, 'Data 定义缺少名称，建议使用 "Data SalesData {"', vscode.DiagnosticSeverity.Error));
            }
            // Unclosed quotes
            const quoteCount = (line.match(/["']/g) || []).length;
            if (quoteCount % 2 !== 0) {
                const range = new vscode.Range(i, line.lastIndexOf('"') || 0, i, line.length);
                diagnostics.push(new vscode.Diagnostic(range, '引号不匹配', vscode.DiagnosticSeverity.Error));
            }
            // Unknown directives
            ...
            if (directiveMatch) {
                ...
                if (!knownDirectives.includes(directiveMatch[1])) {
                    ...
                    diagnostics.push(new vscode.Diagnostic(range, `未知指令 "@${directiveMatch[1]}"`, vscode.DiagnosticSeverity.Warning));
                }
            }
        });
        // Check brace balance
        ...
        if (braceCount !== 0) {
            ...
            diagnostics.push(new vscode.Diagnostic(new vscode.Range(lastLine, 0, lastLine, lines[lastLine]?.length || 0), braceCount > 0 ? `缺少 ${braceCount} 个闭合括号 '}'` : `多余 ${-braceCount} 个闭合括号`, vscode.DiagnosticSeverity.Error));
        }
...
function showPreview(context, document) {
    ...
    if (previewPanel) {
        ...
        previewPanel = vscode.window.createWebviewPanel('cdlPreview', 'CDL Preview', column, {
            enableScripts: true,
...
function getPreviewHtml(echartsOption) {
    const config = vscode.workspace.getConfiguration('cdl.preview');
...
async function compileCDL(document) {
    try {
...
async function exportECharts(document) {
    try {
        const content = document.getText();
