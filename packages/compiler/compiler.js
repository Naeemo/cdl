...
// ===== 简单解析器 =====
function parseCDL(source, errors) {
...
function parseBlock(lines, startIndex, errors) {
    let i = startIndex;
    const line = lines[i].trim();
    // Collect @ directives
    const directives = {};
...
function parseDataBlock(lines, startIndex, directives, errors) {
    let i = startIndex;
    const line = lines[i].trim();
    // Extract name
    ...
    if (directives.timeout)
        config.timeout = Number(directives.timeout);
    if (directives.cache)
        config.cache = Number(directives.cache);
    const block = {
        type: 'data',
        name,
        lang: directives.lang || 'data',
        config,
        query: query.trim(),
    };
    return { block, nextIndex: i };
}
function parseChartBlock(lines, startIndex, hints, errors) {
    let i = startIndex;
    const line = lines[i].trim();
    ...
    while (i < lines.length && braceCount > 0) {
        ...
        if (contentLine.startsWith('use ')) {
            ...
            if (match)
                chart.dataSources.push(match[1]);
        }
            ...
            if (match) {
                const hintName = match[1];
                if (!chart.hints)
                    chart.hints = {};
                chart.hints[hintName] = match[2];
            }
        }
        i++;
    }
...
if (require.main === module) {
    ...
    const source = (0, node_fs_1.readFileSync)(filePath, { encoding: 'utf-8' });
